...
const MediaPlayer = ({ type, src }) => {
    const mediaRef = useRef(null); // Ref para acceder al elemento de audio
    const [volume, setVolume] = useState(1); // Estado para manejar el
    ...
    return (
        < div className="p-4" >
            {type === 'video' ? (<video ref={mediaRef} className="w-full max-w-xl mx-auto"controls={false}> 
            ...
            <div className="flex justify-center items-center space-x-2">
                ...
                <input id="volume" type="range" min="0" max="1" step="0.1"
                    value={volume} onChange={changeVolume} className="w-full max-w-xs" />
            </div>
        </div >
    );
};
